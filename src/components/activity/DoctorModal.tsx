...
export type DoctorVisitMetadata = {
  provider: string;
  reason: string;
  followUpDate?: string;
  cost?: number;
...
export default function DoctorModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      const metadata: DoctorVisitMetadata = {
      ...
      if (!res.ok) {
        throw new Error(data.error || 'Failed to log doctor visit');
      }
      ...
      console.error('Error logging doctor visit:', err);
      setError(err instanceof Error ? err.message : 'Failed to log doctor visit');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    >
      ...
      >
        <Stack spacing={2}>
          ...
          >
            Log doctor visit for {recipientName}
          </Typography>
          ...
          <TextField
            ...
            size="small"
            disabled={isLoading}
            placeholder="e.g., Dr. Johnson at Family Health Clinic"
          />
